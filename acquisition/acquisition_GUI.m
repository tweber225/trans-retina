...
% Last Modified by GUIDE v2.5 28-Sep-2017 13:50:54
...
% TDW: MATLAB requires the next two lines of code to setup command line
% output, don't edit or remove ...
% Choose default command line output for acquisition_GUI
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

% TDW: Here onward, we're free to edit!
...
if get(handles.capStartButton,'Value') == 1
    ...
    if get(handles.prevStartButton,'Value') == 0
        ...
        handles.digitalOutputScan = [1 handles.LEDsToEnable handles.settingsStruct.commFixTarget];
    ...
    while (get(handles.prevStartButton,'Value') == 1) % While the toggle button is DOWN
        ...
        if numFramesAvailNow == numLEDsEnabled % when the number of frames requested are available, gather data and show them
            ...
            if handles.settingsStruct.selectLEDsQuadViewOn == 1 % if quad mode is ON
                ...
                % Show one of these (specified in select LEDs panel) in
                % large LED1Ax frame
                if handles.settingsStruct.commRTFlattening == 1
...
handles.settingsStruct.commRTFlattening = get(handles.RTFlattening,'Value');
...
% end
% if handles.settingsStruct.RTCalcsInCenter == 1
...
% % up the thresholding
% if handles.settingsStruct.RTCalcsInCenter == 0
...
function acquisition_GUI_CloseRequestFcn(hObject, eventdata, handles)
% hObject    handle to acquisition_GUI (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in commFixTarget.
function commFixTarget_Callback(hObject, eventdata, handles)
% Set the new value
handles.settingsStruct.commFixTarget = get(handles.commFixTarget,'Value');
% Toggle fixation target LED
handles.digitalOutputScan(6) = handles.settingsStruct.commFixTarget;
outputSingleScan(handles.NIDaqSession,handles.digitalOutputScan);
% Send along GUI data
guidata(hObject,handles);


% --- Executes on key press with focus on acquisition_GUI and none of its controls.
function acquisition_GUI_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to acquisition_GUI (see GCBO)
...
switch eventdata.Key
    ...
    case 'f' % Toggling realtime image flattening
        ...
        if handles.enteringFilename == 1
            ...
            oldVal = get(handles.commFixTarget,'Value');
            set(handles.commFixTarget,'Value',~oldVal);
...
function commFixTarget_KeyPressFcn(hObject, eventdata, handles)
two_color_image_GUI_KeyPressFcn(hObject, eventdata, handles)

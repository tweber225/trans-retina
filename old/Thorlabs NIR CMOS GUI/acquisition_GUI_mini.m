function varargout = acquisition_GUI_mini(varargin)
% ACQUISITION_GUI_MINI MATLAB code for acquisition_GUI_mini.fig
%      ACQUISITION_GUI_MINI, by itself, creates a new ACQUISITION_GUI_MINI or raises the existing
%      singleton*.
%
%      H = ACQUISITION_GUI_MINI returns the handle to a new ACQUISITION_GUI_MINI or the handle to
%      the existing singleton*.
%
%      ACQUISITION_GUI_MINI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ACQUISITION_GUI_MINI.M with the given input arguments.
% 
%      ACQUISITION_GUI_MINI('Property','Value',...) creates a new ACQUISITION_GUI_MINI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before acquisition_GUI_mini_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to acquisition_GUI_mini_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help acquisition_GUI_mini

% Last Modified by GUIDE v2.5 04-Dec-2018 13:34:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @acquisition_GUI_mini_OpeningFcn, ...
                   'gui_OutputFcn',  @acquisition_GUI_mini_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before acquisition_GUI_mini is made visible.
function acquisition_GUI_mini_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to acquisition_GUI_mini (see VARARGIN)

% Choose default command line output for acquisition_GUI_mini
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes acquisition_GUI_mini wait for user response (see UIRESUME)
...
function varargout = acquisition_GUI_mini_OutputFcn(hObject, eventdata, handles) 
...
if get(hObject,'Value') %... if the button is depressed ...
    ...
    smallFrameSize = min(256,frameHeight);
    if smallFrameSize < 256
        scaleSmallFrameFactor = 256/smallFrameSize;
    else 
        scaleSmallFrameFactor = 1;
    end
    xSm1 = round(frameWidth/2-smallFrameSize/2+1);
    xSm2 = round(frameWidth/2+smallFrameSize/2);
    ySm1 = round(frameHeight/2-smallFrameSize/2+1);
    ySm2 = round(frameHeight/2+smallFrameSize/2);
    ...
    while get(hObject,'Value') % continuously loop while preview button is ON, or until capture is hit
        ...
        % Crop a small section of the sum frame
        % Scale to fill range -pi to pi (because this axis is converted
        % from phase axis in regular GUI version)
        if scaleSmallFrameFactor ==1
            smallSquareFrame = single(scaled8bFrame(ySm1:ySm2,xSm1:xSm2))*(2*pi/256)-pi;
        else
            smallSquareFrame = imresize(single(scaled8bFrame(ySm1:ySm2,xSm1:xSm2))*(2*pi/256)-pi,scaleSmallFrameFactor);
        end
        set(handles.retinaExtra, 'CData', smallSquareFrame);

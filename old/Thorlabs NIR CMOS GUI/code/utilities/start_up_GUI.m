...
% RENDER BLANK ON "EXTRA AXIS"
% note: for displaying phase of frame-by-frame cross power spectrum or for
% showing a small full resolution preview (in mini GUI version)
handles.blankExtraFrame = zeros(round(handles.settings.numberLines/4),'single');
imshow(handles.blankExtraFrame, [-pi, pi], 'Parent', handles.extraAxis);
handles.retinaExtra = get(handles.extraAxis,'Children');
